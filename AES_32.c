...
void get_sbox_value(unsigned char *Word) //T_table안에 있는 sbox 값 추출하기
{
    unsigned int temp;
    temp = Te3[*Word];
    temp = temp >> 24;
...
void Byte_Int_Key_Set(const unsigned char *userKey, AES_KEY *key, int start) // byte 16개 배열을 Key 함수에 저장시키는 함수 start는 key->rdkey[start]의 시작위치
{
    int temp = 0;
...
int AES_set_encrypt_key(const unsigned char *userkey, const int bits, AES_KEY *key) //키생성 함수
{
    ...
    int cnt_i;
    int temp;
    for (cnt_i = 0; cnt_i < Nb * (AES_MAXNR + 1); cnt_i++)
    {
        key->rd_key[cnt_i] = 0x00;// 처음 key 구조체를 초기화 시키기
    }
    Byte_Int_Key_Set(userkey, key, 0); //처음 Masterkey(userkey)를 처음 4개의 배열에 저장시키기
...
void AddRoundKey(unsigned int *state, const AES_KEY *key, int *round)
{
    ...
    for (cnt_i = 0; cnt_i < 4; cnt_i++)
    {
        state[cnt_i] ^= key->rd_key[(4 * (*round)) + cnt_i]; // 키값하고 상태와 XoR 함수 
    }

    *round += 1;
}
void Byte_to_Int(const unsigned char *src8, unsigned int *dst32)
{ //Byte 배열을 Int배열에 저장 시키는 함수
    int cnt_i, temp;
    for (cnt_i = 0; cnt_i < 4; cnt_i++)
    {
        temp = 0;
        temp = src8[4 * cnt_i] << 24;
        temp ^= src8[4 * cnt_i + 1] << 16;
        temp ^= src8[4 * cnt_i + 2] << 8;
        temp ^= src8[4 * cnt_i + 3];
        dst32[cnt_i] = temp;
    }
}
void Int_to_Byte(unsigned int *src32, unsigned char *dst8)
{ //Int 배열을 Byte배열에 저장 시키는 함수
    int cnt_i;
    for (cnt_i = 0; cnt_i < 4; cnt_i++)
    {
        dst8[cnt_i * 4] = src32[cnt_i] >> 24;
        dst8[cnt_i * 4] &= 0x000000ff;
        dst8[cnt_i * 4 + 1] = src32[cnt_i] >> 16;
        dst8[cnt_i * 4 + 1] &= 0x000000ff;
        dst8[cnt_i * 4 + 2] = src32[cnt_i] >> 8;
        dst8[cnt_i * 4 + 2] &= 0x000000ff;
        dst8[cnt_i * 4 + 3] = src32[cnt_i];
        dst8[cnt_i * 4 + 3] &= 0x000000ff;
    }
}

void Te_table(unsigned int *state, const AES_KEY *key, int *round)
{// Te[] 배열을 이용해서, 고속화 구현. 밑의 함수에는 ShiftRow와 Addround함수가 포함되어있다.
    unsigned int *temp;
    temp = (unsigned int *)calloc(4, sizeof(unsigned int));
    temp[0] = Te0[state[0] >> 24] ^ Te1[(state[1] >> 16) & 0xff] ^ Te2[(state[2] >> 8) & 0xff] ^ Te3[state[3] & 0xff] ^ key->rd_key[4 * (*round)];
    temp[1] = Te0[state[1] >> 24] ^ Te1[(state[2] >> 16) & 0xff] ^ Te2[(state[3] >> 8) & 0xff] ^ Te3[state[0] & 0xff] ^ key->rd_key[4 * (*round) + 1];
    temp[2] = Te0[state[2] >> 24] ^ Te1[(state[3] >> 16) & 0xff] ^ Te2[(state[0] >> 8) & 0xff] ^ Te3[state[1] & 0xff] ^ key->rd_key[4 * (*round) + 2];
    temp[3] = Te0[state[3] >> 24] ^ Te1[(state[0] >> 16) & 0xff] ^ Te2[(state[1] >> 8) & 0xff] ^ Te3[state[2] & 0xff] ^ key->rd_key[4 * (*round) + 3];
...
{
    unsigned int state[Nk];
    unsigned char temp[16] = {0x00};
    unsigned char temp2, temp3;
    int cnt_i;
    int round = 0;

    Byte_to_Int(in, state); // Byte단위로 Input배열을 받고, state를 Int단위로 저장
    AddRoundKey(state, key, &round);

    for (cnt_i = 1; cnt_i < AES_MAXNR; cnt_i++)
    {
        Te_table(state, key, &round); //고속화
    }

    Int_to_Byte(state, temp);// Int단위 배열을 Byte배열로 변환시킨다.
    for (cnt_i = 0; cnt_i < 16; cnt_i++)
    {
        get_sbox_value(&temp[cnt_i]);// 그후에 Subbyte 연산.
    }

    // ShiftRow 연산
    temp3 = temp[13]; //2번째 행 1칸 Leftshift
    temp[13] = temp[1];
    temp[1] = temp[5];
    temp[5] = temp[9];
    temp[9] = temp3;

    temp3 = temp[10]; //3번째 행 2칸 Leftshift
    temp2 = temp[14];
    temp[10] = temp[2];
    temp[14] = temp[6];
    temp[2] = temp3;
    ...
    temp3 = temp[7]; // 4번째 행 3칸 Leftshift
    temp[7] = temp[3];
    temp[3] = temp[15];
    temp[15] = temp[11];
    temp[11] = temp3;

    Byte_to_Int(temp, state);// 다시 Byte단위를 Int단위로 합쳐준다
    AddRoundKey(state, key, &round);// 그후에 Addround 함수 연산
    Int_to_Byte(state, out);// 그후에 Byte 단위로 out배열을 정해준다
}
...
{ //AES_encrypt_key 함수에서 생성하는 keyschedule과 반대로 값을 대입해주면 된다.
    ...
    //첫번째와 마지막째의 키를 제외하고 Mixcolumn 연산을 해주면, 암호화와 동일한 Td_table을 구성할 수 있다. 
    //만약 Mixcolumn을 해주지 않으면, Td_table의 맨처음에 Addround함수를 구성해야한다.
    for (cnt_i = 0; cnt_i < 9; cnt_i++)
    {
        temp[0] = key->rd_key[((cnt_i + 1)*4)];
        temp[1] = key->rd_key[((cnt_i + 1)*4) + 1];
        temp[2] = key->rd_key[((cnt_i + 1)*4) + 2];
        temp[3] = key->rd_key[((cnt_i + 1)*4) + 3];
        Int_to_Byte(temp, temp2);
        InvMixColumns(temp2);
        Byte_Int_Key_Set(temp2, key, (cnt_i + 1)*4);
    }
...
{//Td[]배열에는 rsbox값이 포함되어있지 않으므로, rsbox테이블이 필요하다.
    unsigned char temp = rsbox[*Word];
    *Word = temp;
}
void Td_table(unsigned int *state, const AES_KEY *key, int *round)
{//암호화인 Te-table과 똑같은 방식으로 만들어주면 된다. 다른점은 ShiftRow이다.
    unsigned int *temp;
    temp = (unsigned int *)calloc(4, sizeof(unsigned int));
    temp[0] = Td0[state[0] >> 24] ^ Td1[(state[3] >> 16) & 0xff] ^ Td2[(state[2] >> 8) & 0xff] ^ Td3[state[1] & 0xff] ^ key->rd_key[4 * (*round)];
    temp[1] = Td0[state[1] >> 24] ^ Td1[(state[0] >> 16) & 0xff] ^ Td2[(state[3] >> 8) & 0xff] ^ Td3[state[2] & 0xff] ^ key->rd_key[4 * (*round)+1];
    temp[2] = Td0[state[2] >> 24] ^ Td1[(state[1] >> 16) & 0xff] ^ Td2[(state[0] >> 8) & 0xff] ^ Td3[state[3] & 0xff] ^ key->rd_key[4 * (*round)+2];
    temp[3] = Td0[state[3] >> 24] ^ Td1[(state[2] >> 16) & 0xff] ^ Td2[(state[1] >> 8) & 0xff] ^ Td3[state[0] & 0xff] ^ key->rd_key[4 * (*round)+3];
...
{//암호화랑 동일하게 복호화 해주면 된다.
    ...
    Byte_to_Int(in, state);
    AddRoundKey(state, key, &round);

    for (cnt_i = 1; cnt_i < AES_MAXNR; cnt_i++)
    {
        Td_table(state, key, &round);
    }

    Int_to_Byte(state, temp);
    temp3 = temp[13]; //2번째 행 1칸 Rightshift
    temp[13] = temp[9];
    temp[9] = temp[5];
    temp[5] = temp[1];
    temp[1] = temp3;

    temp3 = temp[10]; //2번째 행 2칸 Rightshift
    temp2 = temp[14];
    temp[10] = temp[2];
    temp[14] = temp[6];
    temp[2] = temp3;
    ...
    temp3 = temp[3]; //2번째 행 3칸 Rightshift
    temp[3] = temp[7];
    temp[7] = temp[11];
    temp[11] = temp[15];
    temp[15] = temp3;
    ...
    for (cnt_i = 0; cnt_i < 16; cnt_i++)
    {
        get_rsbox_value(&temp[cnt_i]);
    }
    Byte_to_Int(temp, state);
    AddRoundKey(state, key, &round);
    Int_to_Byte(state, out);
}

void InvMixColumns(unsigned char *state)
{// 복호화 키의 연산에 필요한 InvMixcolumn연산이다. 최적화를 통해서 xtime함수 호출을 총 9번 한다.
    unsigned char *temp;
